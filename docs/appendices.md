...
| Prometheus target missing | PodMonitor matched nothing | The monitor targets the sidecar's `http-envoy-prom` port, so a pod with no sidecar has nothing to scrape. Check `READY 2/2` first, then labels ([§9.6](phase-4-service-mesh.md#96-the-metrics-problem-you-just-created)). |
